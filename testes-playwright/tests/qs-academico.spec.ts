...
test.describe('Validação de Notas', () => {
  ...
  test('deve aceitar nota nos limites válidos (0 e 10)', async ({ page }) => {
    ...
    await expect(page.locator('#tabela-alunos tbody tr')).toHaveCount(1);
    await expect(page.getByLabel('Teste Limites')).toBeVisible();
  });
...
test.describe('Busca por Nome', () => {
  ...
  test('deve exibir apenas o aluno correspondente ao termo buscado', async ({ page }) => {
    ...
    await expect(page.getByLabel('Ana Souza')).toBeVisible();
    await expect(page.getByLabel('Carlos Lima')).not.toBeVisible();
  ...
  test('deve restaurar todos os alunos ao limpar o filtro', async ({ page }) => {
    ...
    await expect(page.locator('#tabela-alunos tbody tr')).toHaveCount(2);
    await expect(page.getByLabel('Ana Souza')).toBeVisible();
    await expect(page.getByLabel('Carlos Lima')).toBeVisible();
  });
  ...
  test('deve exibir mensagem de nenhum resultado para busca inexistente', async ({ page }) => {
    ...
    await expect(page.locator('#tabela-alunos tbody td.texto-central')).toBeVisible();
    await expect(page.getByLabel('Ana Souza')).not.toBeVisible();
    await expect(page.getByLabel('Carlos Lima')).not.toBeVisible();
  });
...
test.describe('Exclusão de Alunos', () => {
  ...
  test('deve excluir o único aluno e deixar a tabela vazia', async ({ page }) => {
    ...
    await expect(page.locator('#tabela-alunos tbody tr')).toHaveCount(1);
    await expect(page.getByLabel('Lucas Pereira')).toBeVisible();
    ...
    await expect(page.locator('#tabela-alunos tbody td.texto-central')).toBeVisible();
    await expect(page.getByLabel('Lucas Pereira')).not.toBeVisible();
  });
  ...
  test('deve excluir apenas o aluno selecionado, mantendo os demais', async ({ page }) => {
    ...
    await expect(page.locator('#tabela-alunos tbody tr')).toHaveCount(1);
    await expect(page.getByLabel('Roberto Alves')).toBeVisible();
    await expect(page.getByLabel('Mariana Faria')).not.toBeVisible();
  });
  ...
  test('deve limpar todos os alunos com o botão "Limpar Tudo"', async ({ page }) => {
    ...
    await expect(page.locator('#tabela-alunos tbody td.texto-central')).toBeVisible();
    await expect(page.getByLabel('Fernanda Costa')).not.toBeVisible();
    await expect(page.getByLabel('Diego Martins')).not.toBeVisible();
  });
  ...
  test('não deve excluir nenhum aluno ao cancelar o "Limpar Tudo"', async ({ page }) => {
    ...
    await expect(page.locator('#tabela-alunos tbody tr')).toHaveCount(1);
    await expect(page.getByLabel('Patrícia Nunes')).toBeVisible();
  });
...
test.describe('Situação — Aprovado', () => {
  ...
  test('deve exibir "Aprovado" para média exatamente igual a 7 (limite inferior)', async ({ page }) => {
    ...
    await expect(linha.getByLabel('Aprovado', { exact: true })).toBeVisible();
  });
  ...
  test('não deve exibir "Aprovado" para média abaixo de 7 (fronteira inferior)', async ({ page }) => {
    ...
    const linha = page.locator('#tabela-alunos tbody tr').first();
    await expect(linha.getByTLabel('Aprovado', { exact: true })).not.toBeVisible();
  });
...
test.describe('Situação — Reprovado', () => {
  ...
  test('deve exibir "Reprovado" para média exatamente igual a 4.99 (fronteira superior)', async ({ page }) => {
    ...
    await expect(linha.getByTLabel('Reprovado', { exact: true })).toBeVisible();
  });
  ...
  test('deve exibir "Reprovado" para média bem abaixo de 5', async ({ page }) => {
    ...
    await expect(linha.getByLabel('Reprovado', { exact: true })).toBeVisible();
  });
  ...
  test('não deve exibir "Reprovado" para média igual a 5 (fronteira com Recuperação)', async ({ page }) => {
    ...
    await expect(linha.getByLabel('Reprovado', { exact: true })).not.toBeVisible();
  });
...
test.describe('Múltiplos Cadastros', () => {
  ...
  test('deve preservar os dados de cada aluno após cadastros consecutivos', async ({ page }) => {
    ...
    await expect(page.locator('#tabela-alunos tbody').getByText('Beatriz Souza')).toBeVisible();
    await expect(page.locator('#tabela-alunos tbody').getByText('Eduardo Lima')).toBeVisible();
    await expect(page.locator('#tabela-alunos tbody').getByText('Gabriela Matos')).toBeVisible();
  ...
  test('deve manter a ordem de inserção dos alunos na tabela', async ({ page }) => {
    ...
    await expect(linhas.nth(0).locator('td').first()).toHaveText('Primeiro Silva');
    await expect(linhas.nth(1).locator('td').first()).toHaveText('Segundo Santos');
    await expect(linhas.nth(2).locator('td').first()).toHaveText('Terceiro Costa');
  });
